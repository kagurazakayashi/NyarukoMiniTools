...
void ref() {
	...
	char str[1001] = { '\0' };
	while (1) {
		int scanfcode = scanf("%1000s", &str); // 讀取最多 1000 個字符的元串
...
void ltime() {
	...
	while (1) {
		...
		printf("%4d-%02d-%02d %02d:%02d:%02d\n", 1900 + p.tm_year, 1 + p.tm_mon,
			p.tm_mday, p.tm_hour, p.tm_min, p.tm_sec); // 顯示當前時間
...
int main(int argc, char* argv[]) {
	...
	if (argc >= 2) {
		if (strcmp(aStr, "/?") == 0 || argcmp(aStr, 'H') == 1 ||
			strcmp(aStr, "--help") == 0) {
			...
			printf("Written by Kagurazaka Yashi. "
				"https://github.com/kagurazakayashi/NyarukoMiniTools\n");

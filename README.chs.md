...
- `选项`:
  - `/D` : 每秒输出剩余时间（秒）
  - `/P` : 使用高精度计时器（更消耗 CPU），不建议和 `/D` 同时使用。
  - `/?` : 显示此帮助信息
  - `/V` : 显示版本信息
...
- 模式:
  ...
  - `/?`: 显示此帮助信息
  - `/V`: 显示版本信息
- 如果不提供文件路径，将进入接受用户键盘输入模式。
  - 按两下回车时结束输入并运行统计，
  - 如果什么都不输入直接按回车，将退出程序。

## ![sizestat](sizestat/icon1.ico) sizestat.exe

按文件大小区间寻找文件，可用于寻找占用空间较大的文件在哪。

### sizestat:使用

- 使用:  `sizestat <文件夹路径> [/R] [/I] [区间列表]`
- 模式:
  - `/R`: 扫描子文件夹
  - `/I`: 显示详细扫描位置信息
- 区间列表:
  - 格式: `<从字节数>-<到字节数>,<从字节数>-<到字节数>,...`
  - 示例:
    - 默认值: `512-1023,1024-2047`
    - 常用值: 根据 Windows 格式化文件系统的常见分配单元大小:
...
#define UNITINFO_NTFS  "512      1024      2048      4096      8192       16KB        32KB        64KB         128KB         256KB         512KB          1024KB          2048KB" // (默认值)
